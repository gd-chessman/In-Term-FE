...
export const getProducts = async (params: { page?: number; limit?: number; search?: string; product_name?: string; product_code?: string; product_description?: string; category_id?: number; product_status?: string; min_price?: number; max_price?: number; tag_id?: number; tag_ids?: number[]; sort_by?: string; sort_order?: 'asc' | 'desc';
} = {}) => {
    try {
        const { page = 1, limit = 10, search, product_name, product_code, category_id, product_description, product_status, min_price, max_price, tag_id, tag_ids, sort_by, sort_order } = params;
